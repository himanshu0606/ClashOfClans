...
class Barbarian:
    ...
    def break_wall(self, x, y, V, King):
        ...
        self.attack_target_wall(target, King)

    def attack_target_wall(self, target, King):
        if (self.alive == False):
            return
        ...
        if target.health <= 0:
            ...
            if (target.level >= 3):
                pos = target.position
                troops_wall = barbarians + archers + stealth_archers+[King]
                for troop_wall in troops_wall:
                    if (abs(troop_wall.position[0]-pos[0])+abs(troop_wall.position[1]-pos[1]) <= 2):
                        troop_wall.deal_damage(200)
            target.destroy()
...
class Healer:
    ...
    def heal_troop(self, troops):
        if (self.alive == False):
            return
        for troop in troops:
            if(troop.alive):
                troop.health = min(
                    troop.max_health, troop.health+self.heal_strength)

    def move(self, closest_troop, King):
        if (self.alive == False):
            return
        ...
        if (r+c <= 7):
            heal_troops = []
            troops = barbarians + archers + dragons + \
                balloons+stealth_archers+[King]
            for troop in troops:
                if (abs(troop.position[0]-pos[0])+abs(troop.position[1]-pos[1]) <= 1):
                    heal_troops.append(troop)
            self.heal_troop(heal_troops)
        elif (r == 0):
            if (pos[1] > self.position[1]):
                for i in range(self.speed):
                    r = self.position[0]
...
def search_for_closest_troop(pos, King):
    troops = barbarians + archers + dragons + balloons+stealth_archers+[King]
    closest_wounded_troop = None
    closest_troop = None
    max_dist_wounded = 1000
    max_dist = 1000
    for troop in troops:
        if(troop.alive):
            if (troop.health < troop.max_health):
            
                if ((abs(troop.position[0]-pos[0]) + abs(troop.position[1]-pos[1])) <= max_dist_wounded):
                    closest_wounded_troop = troop
                    max_dist_wounded = (
                        abs(troop.position[0]-pos[0]) + abs(troop.position[1]-pos[1]))

            if ((abs(troop.position[0]-pos[0]) + abs(troop.position[1]-pos[1])) <= max_dist):
                closest_troop = troop
                max_dist = (
                    abs(troop.position[0]-pos[0]) + abs(troop.position[1]-pos[1]))
    if closest_wounded_troop is not None:
